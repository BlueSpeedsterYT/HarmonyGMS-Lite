...
function player_is_looking(phase)
{
	switch (phase)
	{
		...
		case PHASE.STEP:
		{
			...
			{
				if (panning_oy > CAMERA_PAN_TARGET_UP) panning_oy -= 2;
			}
			break;
		}
		case PHASE.EXIT:
...
function player_is_crouching(phase)
{
	switch (phase)
	{
		...
		case PHASE.STEP:
		{
			...
			{
				if (panning_oy < CAMERA_PAN_TARGET_DOWN) panning_oy += 2;
			}
			break;
		}
		case PHASE.EXIT:
	...
	switch (phase)
	{
		...
		case PHASE.STEP:
		{
			...
			if (input_button.jump.pressed)
			{
				...
				// Sound
				var rev_sound = sound_play(sfxRoll);
				audio_sound_pitch(rev_sound, 1 + spin_dash_charge * 0.0625);
			}
			else 
			{
				spin_dash_charge *= SPIN_DASH_ATROPHY;
			}
			break;
		}
		case PHASE.EXIT:
